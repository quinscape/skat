...
import de.fforw.skat.runtime.message.IncomingMessage;
import de.fforw.skat.runtime.message.IncomingMessageHandler;
    ...
    extends TextWebSocketHandler
{
    ...
    public SkatWebSocketHandler(Set<GraphQLMessageHandler> handlers)
    {

        log.info("Starting SkatWebSocketHandler, handlers = {}", handlers);

        for (GraphQLMessageHandler handler : handlers)
        {
     ...
     *
     * @param connectionId      unique connection id (*not* the websocket session id)
     * @param auth              application auth
     */
    public void register(String connectionId, AppAuthentication auth)
    {
        connections.put(connectionId, new SkatClientConnection(connectionId, auth));
    }
